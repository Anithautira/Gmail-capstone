...
function UserProfile() {
    ...
    async function submitClick() {
        ...
        if (nameError.innerText === "" && passwordError.innerText === "" && securityCodeError.innerText === "") {
            ...
            await axios.put(`http://localhost:3000/updateUser?email=${userData.email}&securityCode=${security}`, newPassword)
                .then((res) => {
                    ...
                    async function getUserData() {
                        let passwordInput = '';
                        await axios.get(`http://localhost:3000/login?email=${email.value}&password=${passwordInput}`)
                            .then((res) => {
